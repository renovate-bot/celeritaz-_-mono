...
const EmergencyCard = ({ data }: { data: PatientCompleteData }) => {
  return (
        ...
        {data?.emergencyDetailsData && data?.emergencyDetailsData.length > 0 ? (
          data?.emergencyDetailsData?.map((item, index) => (
            <div className="flex flex-col gap-2" key={index}>
              <p className="text-primary text-xs font-semibold">{`Contact #${index + 1}`}</p>
              <LabelValue
                label="Full Name"
              ...
              <LabelValue label="Email Address" value={item.email} />
